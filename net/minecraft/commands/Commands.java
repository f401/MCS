...
public class Commands {
      ...
      try {
         ...
         if (contextchain != null) {
            executeCommandInContext(commandsourcestack, (p_308343_) -> {
               ExecutionContext.queueInitialCommandExecution(p_308343_, pCommand, contextchain, commandsourcestack, CommandResultCallback.EMPTY);
            });
         }
   ...
   private static ContextChain<CommandSourceStack> finishParsing(ParseResults<CommandSourceStack> pParseResults, String pCommand, CommandSourceStack pSource) {
      try {
         ...
         return ContextChain.tryFlatten(pParseResults.getContext().build(pCommand)).orElseThrow(() -> {
            return CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownCommand().createWithContext(pParseResults.getReader());
         });
