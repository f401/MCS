...
public class ExecutionContext<T> implements AutoCloseable {
   ...
   public static <T extends ExecutionCommandSource<T>> void queueInitialFunctionCall(ExecutionContext<T> pExecutionContext, InstantiatedFunction<T> pFunction, T pSource, CommandResultCallback pReturnValueConsumer) {
      pExecutionContext.queueNext(new CommandQueueEntry<>(createTopFrame(pExecutionContext, pReturnValueConsumer), (new CallFunction<>(pFunction, pSource.callback(), false)).bind(pSource)));
   }

   public static <T extends ExecutionCommandSource<T>> void queueInitialCommandExecution(ExecutionContext<T> pExecutionContext, String pCommandInput, ContextChain<T> pCommand, T pSource, CommandResultCallback pReturnValueConsumer) {
   ...
   public Frame.FrameControl frameControlForDepth(int pDepth) {
      return () -> {
         this.discardAtDepthOrHigher(pDepth);
      };
   }
