...
class FunctionBuilder<T extends ExecutionCommandSource<T>> {
   ...
   public CommandFunction<T> build(ResourceLocation pId) {
      return (CommandFunction<T>)(this.macroEntries != null ? new MacroFunction<>(pId, this.macroEntries, this.macroArguments) : new PlainTextFunction<>(pId, this.plainEntries));
   }
}