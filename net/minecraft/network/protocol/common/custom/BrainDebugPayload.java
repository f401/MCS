...
public record BrainDebugPayload(BrainDebugPayload.BrainDump brainDump) implements CustomPacketPayload {
   ...
   public BrainDebugPayload(FriendlyByteBuf pBuffer) {
   ...
   public static record BrainDump(UUID uuid, int id, String name, String profession, int xp, float health, float maxHealth, Vec3 pos, String inventory, @Nullable Path path, boolean wantsGolem, int angerLevel, List<String> activities, List<String> behaviors, List<String> memories, List<String> gossips, Set<BlockPos> pois, Set<BlockPos> potentialPois) {
      public BrainDump(FriendlyByteBuf pBuffer) {
         this(pBuffer.readUUID(), pBuffer.readInt(), pBuffer.readUtf(), pBuffer.readUtf(), pBuffer.readInt(), pBuffer.readFloat(), pBuffer.readFloat(), pBuffer.readVec3(), pBuffer.readUtf(), pBuffer.readNullable(Path::createFromStream), pBuffer.readBoolean(), pBuffer.readInt(), pBuffer.readList(FriendlyByteBuf::readUtf), pBuffer.readList(FriendlyByteBuf::readUtf), pBuffer.readList(FriendlyByteBuf::readUtf), pBuffer.readList(FriendlyByteBuf::readUtf), pBuffer.readCollection(HashSet::new, FriendlyByteBuf::readBlockPos), pBuffer.readCollection(HashSet::new, FriendlyByteBuf::readBlockPos));
      }
      ...
      public void write(FriendlyByteBuf pBuffer) {
         ...
         pBuffer.writeNullable(this.path, (p_297936_, p_301045_) -> {
            p_301045_.writeToStream(p_297936_);
         });
