...
public abstract class RecipeProvider implements DataProvider {
      ...
      this.buildRecipes(new RecipeOutput() {
         public void accept(ResourceLocation p_312383_, Recipe<?> p_312007_, @Nullable ResourceLocation advancementId, @Nullable com.google.gson.JsonElement advancement) {
            if (!set.add(p_312383_)) {
               ...
               list.add(DataProvider.saveStable(pOutput, Recipe.CODEC, p_312007_, RecipeProvider.this.recipePathProvider.json(p_312383_)));
               if (advancement != null) {
   ...
   protected CompletableFuture<?> buildAdvancement(CachedOutput pOutput, AdvancementHolder pAdvancementBuilder) {
      return DataProvider.saveStable(pOutput, Advancement.CODEC, pAdvancementBuilder.value(), this.advancementPathProvider.json(pAdvancementBuilder.id()));
   }
   ...
   protected static void generateForEnabledBlockFamilies(RecipeOutput pRecipeOutput, FeatureFlagSet pEnabledFeatures) {
      BlockFamilies.getAllFamilies().filter(BlockFamily::shouldGenerateRecipe).forEach((p_308528_) -> {
         generateRecipes(pRecipeOutput, p_308528_, pEnabledFeatures);
      });
   }
   ...
   private static <T extends AbstractCookingRecipe> void simpleCookingRecipe(RecipeOutput pRecipeOutput, String pCookingMethod, RecipeSerializer<T> pCookingSerializer, AbstractCookingRecipe.Factory<T> pRecipeFactory, int pCookingTime, ItemLike pMaterial, ItemLike pResult, float pExperience) {
      SimpleCookingRecipeBuilder.generic(Ingredient.of(pMaterial), RecipeCategory.FOOD, pResult, pExperience, pCookingTime, pCookingSerializer, pRecipeFactory).unlockedBy(getHasName(pMaterial), has(pMaterial)).save(pRecipeOutput, getItemName(pResult) + "_from_" + pCookingMethod);
   }
   ...
   protected static void waxRecipes(RecipeOutput pRecipeOutput, FeatureFlagSet pRequiredFeatures) {
      HoneycombItem.WAXABLES.get().forEach((p_308519_, p_308520_) -> {
         if (p_308520_.requiredFeatures().isSubsetOf(pRequiredFeatures)) {
            ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, p_308520_).requires(p_308519_).requires(Items.HONEYCOMB).group(getItemName(p_308520_)).unlockedBy(getHasName(p_308519_), has(p_308519_)).save(pRecipeOutput, getConversionRecipeName(p_308520_, Items.HONEYCOMB));
         }
      });
   }

   protected static void grate(RecipeOutput pRecipeOutput, Block pGrateBlock, Block pMaterial) {
      ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, pGrateBlock, 4).define('M', pMaterial).pattern(" M ").pattern("M M").pattern(" M ").unlockedBy(getHasName(pMaterial), has(pMaterial)).save(pRecipeOutput);
   }

   protected static void copperBulb(RecipeOutput pRecipeOutput, Block pBulbBlock, Block pMaterial) {
      ShapedRecipeBuilder.shaped(RecipeCategory.REDSTONE, pBulbBlock, 4).define('C', pMaterial).define('R', Items.REDSTONE).define('B', Items.BLAZE_ROD).pattern(" C ").pattern("CBC").pattern(" R ").unlockedBy(getHasName(pMaterial), has(pMaterial)).save(pRecipeOutput);
   }
   ...
   protected static void generateRecipes(RecipeOutput pRecipeOutput, BlockFamily pBlockFamily, FeatureFlagSet pRequiredFeatures) {
      pBlockFamily.getVariants().forEach((p_308524_, p_308525_) -> {
   ...
   private static Criterion<EnterBlockTrigger.TriggerInstance> insideOf(Block pBlock) {
      return CriteriaTriggers.ENTER_BLOCK.createCriterion(new EnterBlockTrigger.TriggerInstance(Optional.empty(), Optional.of(pBlock.builtInRegistryHolder()), Optional.empty()));
   }
   ...
   private static Criterion<InventoryChangeTrigger.TriggerInstance> inventoryTrigger(ItemPredicate... pPredicates) {
      return CriteriaTriggers.INVENTORY_CHANGED.createCriterion(new InventoryChangeTrigger.TriggerInstance(Optional.empty(), InventoryChangeTrigger.TriggerInstance.Slots.ANY, List.of(pPredicates)));
   }
