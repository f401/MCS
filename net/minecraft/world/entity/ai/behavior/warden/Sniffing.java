...
public class Sniffing<E extends Warden> extends Behavior<E> {
   ...
   protected void stop(ServerLevel pLevel, E pEntity, long pGameTime) {
      ...
      pEntity.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(pEntity::canTargetEntity).ifPresent((p_309119_) -> {
         if (pEntity.closerThan(p_309119_, 6.0D, 20.0D)) {
            pEntity.increaseAngerAt(p_309119_);
         }

         if (!pEntity.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
            WardenAi.setDisturbanceLocation(pEntity, p_309119_.blockPosition());
         }
