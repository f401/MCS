...
public class Sniffing<E extends Warden> extends Behavior<E> {
   ...
   protected void stop(ServerLevel pLevel, E pEntity, long pGameTime) {
      ...
      pEntity.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(pEntity::canTargetEntity).ifPresent((p_296747_) -> {
         if (pEntity.closerThan(p_296747_, 6.0D, 20.0D)) {
            pEntity.increaseAngerAt(p_296747_);
         }

         if (!pEntity.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
            WardenAi.setDisturbanceLocation(pEntity, p_296747_.blockPosition());
         }
