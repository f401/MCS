...
public class InteractWithDoor {
   ...
   private static boolean areOtherMobsComingThroughDoor(LivingEntity pEntity, BlockPos pPos, Optional<List<LivingEntity>> pNearestLivingEntities) {
      return pNearestLivingEntities.isEmpty() ? false : pNearestLivingEntities.get().stream().filter((p_296684_) -> {
         return p_296684_.getType() == pEntity.getType();
      }).filter((p_296682_) -> {
         return pPos.closerToCenterThan(p_296682_.position(), 2.0D);
      }).anyMatch((p_258454_) -> {
