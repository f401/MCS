...
public class InteractWithDoor {
   ...
   private static boolean areOtherMobsComingThroughDoor(LivingEntity pEntity, BlockPos pPos, Optional<List<LivingEntity>> pNearestLivingEntities) {
      return pNearestLivingEntities.isEmpty() ? false : pNearestLivingEntities.get().stream().filter((p_309047_) -> {
         return p_309047_.getType() == pEntity.getType();
      }).filter((p_309049_) -> {
         return pPos.closerToCenterThan(p_309049_.position(), 2.0D);
      }).anyMatch((p_258454_) -> {
