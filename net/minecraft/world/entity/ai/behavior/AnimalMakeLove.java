...
public class AnimalMakeLove extends Behavior<Animal> {
   ...
   protected boolean canStillUse(ServerLevel pLevel, Animal pEntity, long pGameTime) {
      if (!this.hasBreedTargetOfRightType(pEntity)) {
         ...
         return animal.isAlive() && pEntity.canMate(animal) && BehaviorUtils.entityIsVisible(pEntity.getBrain(), animal) && pGameTime <= this.spawnChildAtTime && !pEntity.isPanicking() && !animal.isPanicking();
      }
   }
   ...
   private Optional<? extends Animal> findValidBreedPartner(Animal pAnimal) {
      return pAnimal.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().findClosest((p_296665_) -> {
         if (p_296665_.getType() == this.partnerType && p_296665_ instanceof Animal animal) {
            if (pAnimal.canMate(animal) && !animal.isPanicking()) {
               return true;
            }
         }
