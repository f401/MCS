...
public class LongJump extends Behavior<Breeze> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel pLevel, Breeze pOwner) {
      if (!pOwner.onGround() && !pOwner.isInWater()) {
         ...
         LivingEntity livingentity = pOwner.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).orElse((LivingEntity)null);
   ...
   protected void start(ServerLevel pLevel, Breeze pEntity, long pGameTime) {
      ...
      pEntity.getBrain().getMemory(MemoryModuleType.BREEZE_JUMP_TARGET).ifPresent((p_311106_) -> {
         pEntity.lookAt(EntityAnchorArgument.Anchor.EYES, p_311106_.getCenter());
      });
   }

   protected void tick(ServerLevel pLevel, Breeze pOwner, long pGameTime) {
      if (finishedInhaling(pOwner)) {
         Vec3 vec3 = pOwner.getBrain().getMemory(MemoryModuleType.BREEZE_JUMP_TARGET).flatMap((p_311994_) -> {
            return calculateOptimalJumpVector(pOwner, pOwner.getRandom(), Vec3.atBottomCenterOf(p_311994_));
         }).orElse((Vec3)null);
         if (vec3 == null) {
   ...
   private static BlockPos snapToSurface(LivingEntity pOwner, Vec3 pTargetPos) {
      ClipContext clipcontext = new ClipContext(pTargetPos, pTargetPos.relative(Direction.DOWN, 10.0D), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, pOwner);
      HitResult hitresult = pOwner.level().clip(clipcontext);
      if (hitresult.getType() == HitResult.Type.BLOCK) {
         return BlockPos.containing(hitresult.getLocation()).above();
      } else {
         ClipContext clipcontext1 = new ClipContext(pTargetPos, pTargetPos.relative(Direction.UP, 10.0D), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, pOwner);
   ...
   public static boolean hasLineOfSight(Breeze pBreeze, Vec3 pTargetPos) {
      ...
      if (pTargetPos.distanceTo(vec3) > 50.0D) {
         return false;
      } else {
         return pBreeze.level().clip(new ClipContext(vec3, pTargetPos, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, pBreeze)).getType() == HitResult.Type.MISS;
      }
   }

   private static boolean outOfAggroRange(Breeze pBreeze, LivingEntity pTarget) {
      return !pTarget.closerThan(pBreeze, 24.0D);
   }
