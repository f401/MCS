...
public class Drowned extends Zombie implements RangedAttackMob {
   ...
   public static boolean checkDrownedSpawnRules(EntityType<Drowned> pDrowned, ServerLevelAccessor pServerLevel, MobSpawnType pMobSpawnType, BlockPos pPos, RandomSource pRandom) {
      if (!pServerLevel.getFluidState(pPos.below()).is(FluidTags.WATER) && !MobSpawnType.isSpawner(pMobSpawnType)) {
         ...
         boolean flag = pServerLevel.getDifficulty() != Difficulty.PEACEFUL && (MobSpawnType.ignoresLightRequirements(pMobSpawnType) || isDarkEnoughToSpawn(pServerLevel, pPos, pRandom)) && (MobSpawnType.isSpawner(pMobSpawnType) || pServerLevel.getFluidState(pPos).is(FluidTags.WATER));
