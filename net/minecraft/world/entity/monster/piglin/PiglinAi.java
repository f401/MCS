...
public class PiglinAi {
   ...
   public static void angerNearbyPiglins(Player pPlayer, boolean pAngerOnlyIfCanSee) {
      ...
      list.stream().filter(PiglinAi::isIdle).filter((p_34881_) -> {
         ...
         if (p_309198_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
            setAngerTargetToNearestTargetablePlayerIfFound(p_309198_, pPlayer);
         } else {
            setAngerTarget(p_309198_, pPlayer);
         }
   ...
   protected static void wasHurtBy(Piglin pPiglin, LivingEntity pTarget) {
      if (!(pTarget instanceof Piglin)) {
         ...
         getAvoidTarget(pPiglin).ifPresent((p_309201_) -> {
            if (p_309201_.getType() != pTarget.getType()) {
   ...
   protected static void broadcastAngerTarget(AbstractPiglin pPiglin, LivingEntity pTarget) {
      getAdultPiglins(pPiglin).forEach((p_309194_) -> {
         if (pTarget.getType() != EntityType.HOGLIN || p_309194_.canHunt() && ((Hoglin)pTarget).canBeHunted()) {
            setAngerTargetIfCloserThanCurrent(p_309194_, pTarget);
         }
      });
   }
