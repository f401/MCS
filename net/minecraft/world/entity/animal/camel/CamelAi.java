...
public class CamelAi {
   ...
   private static void initIdleActivity(Brain<Camel> pBrain) {
      pBrain.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, new AnimalMakeLove(EntityType.CAMEL, 1.0F)), Pair.of(2, new RunOne<>(ImmutableList.of(Pair.of(new FollowTemptation((p_250812_) -> {
         return 2.5F;
      }, (p_296810_) -> {
         return p_296810_.isBaby() ? 2.5D : 3.5D;
   ...
   public static class RandomSitting extends Behavior<Camel> {
      ...
      protected boolean checkExtraStartConditions(ServerLevel pLevel, Camel pOwner) {
         return !pOwner.isInWater() && pOwner.getPoseTime() >= (long)this.minimalPoseTicks && !pOwner.isLeashed() && pOwner.onGround() && !pOwner.hasControllingPassenger() && pOwner.canCamelChangePose();
      }
