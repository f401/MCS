...
public class JigsawPlacement {
   ...
   public static Optional<Structure.GenerationStub> addPieces(Structure.GenerationContext pContext, Holder<StructureTemplatePool> pStartPool, Optional<ResourceLocation> pStartJigsawName, int pMaxDepth, BlockPos pPos, boolean pUseExpansionHack, Optional<Heightmap.Types> pProjectStartToHeightmap, int pMaxDistanceFromCenter, PoolAliasLookup pAliasLookup) {
      ...
      StructureTemplatePool structuretemplatepool = pStartPool.unwrapKey().flatMap((p_309329_) -> {
         return registry.getOptional(pAliasLookup.lookup(p_309329_));
      ...
      if (structurepoolelement == EmptyPoolElement.INSTANCE) {
         ...
         return Optional.of(new Structure.GenerationStub(new BlockPos(i, i1, j), (p_309345_) -> {
            ...
            if (pMaxDepth > 0) {
               ...
               addPieces(pContext.randomState(), pMaxDepth, pUseExpansionHack, chunkgenerator, structuretemplatemanager, levelheightaccessor, worldgenrandom, registry, poolelementstructurepiece, list, voxelshape, pAliasLookup);
   ...
   private static void addPieces(RandomState pRandomState, int pMaxDepth, boolean pUseExpansionHack, ChunkGenerator pChunkGenerator, StructureTemplateManager pStructureTemplateManager, LevelHeightAccessor pLevel, RandomSource pRandom, Registry<StructureTemplatePool> pPools, PoolElementStructurePiece p_227219_, List<PoolElementStructurePiece> pPieces, VoxelShape pFree, PoolAliasLookup pAliasLookup) {
      ...
      while(jigsawplacement$placer.placing.hasNext()) {
         ...
         jigsawplacement$placer.tryPlacingChildren(jigsawplacement$piecestate.piece, jigsawplacement$piecestate.free, jigsawplacement$piecestate.depth, pUseExpansionHack, pLevel, pRandomState, pAliasLookup);
      }
   ...
   public static boolean generateJigsaw(ServerLevel pLevel, Holder<StructureTemplatePool> pStartPool, ResourceLocation pStartJigsawName, int pMaxDepth, BlockPos pPos, boolean pKeepJigsaws) {
      ...
      Optional<Structure.GenerationStub> optional = addPieces(structure$generationcontext, pStartPool, Optional.of(pStartJigsawName), pMaxDepth, pPos, false, Optional.empty(), 128, PoolAliasLookup.EMPTY);
   ...
   static final class Placer {
      ...
      void tryPlacingChildren(PoolElementStructurePiece pPiece, MutableObject<VoxelShape> pFree, int pDepth, boolean pUseExpansionHack, LevelHeightAccessor pLevel, RandomState pRandomState, PoolAliasLookup pPoolAliasLookup) {
                           ...
                           for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo1 : list1) {
                              if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                 ...
                                 if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                    ...
                                    pPiece.addJunction(new JigsawJunction(blockpos2.getX(), i3 - j + j3, blockpos2.getZ(), l1, structuretemplatepool$projection1));
                                    ...
                                    if (pDepth + 1 <= this.maxDepth) {
                                       JigsawPlacement.PieceState jigsawplacement$piecestate = new JigsawPlacement.PieceState(poolelementstructurepiece, mutableobject1, pDepth + 1);
      ...
      private static ResourceKey<StructureTemplatePool> readPoolKey(StructureTemplate.StructureBlockInfo pBlockInfo, PoolAliasLookup pAliasLookup) {
         CompoundTag compoundtag = Objects.requireNonNull(pBlockInfo.nbt(), () -> {
            return pBlockInfo + " nbt was null";
         });
