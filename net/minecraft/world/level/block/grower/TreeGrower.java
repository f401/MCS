...
public final class TreeGrower {
   ...
   private ResourceKey<ConfiguredFeature<?, ?>> getConfiguredFeature(RandomSource pRandom, boolean pFlowers) {
      ...
      return pFlowers && this.flowers.isPresent() ? this.flowers.get() : this.tree.orElse((ResourceKey<ConfiguredFeature<?, ?>>)null);
   }

   @Nullable
   private ResourceKey<ConfiguredFeature<?, ?>> getConfiguredMegaFeature(RandomSource pRandom) {
      return this.secondaryMegaTree.isPresent() && pRandom.nextFloat() < this.secondaryChance ? this.secondaryMegaTree.get() : this.megaTree.orElse((ResourceKey<ConfiguredFeature<?, ?>>)null);
   }
   ...
   public boolean growTree(ServerLevel pLevel, ChunkGenerator pChunkGenerator, BlockPos pPos, BlockState pState, RandomSource pRandom) {
      ...
      if (resourcekey1 == null) {
         ...
         Holder<ConfiguredFeature<?, ?>> holder1 = pLevel.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).getHolder(resourcekey1).orElse((Holder.Reference<ConfiguredFeature<?, ?>>)null);
         if (holder1 == null) {
            return false;
         } else {
   ...
   private static boolean isTwoByTwoSapling(BlockState pState, BlockGetter pLevel, BlockPos pPos, int pXOffset, int pYOffset) {
      ...
      return pLevel.getBlockState(pPos.offset(pXOffset, 0, pYOffset)).is(block) && pLevel.getBlockState(pPos.offset(pXOffset + 1, 0, pYOffset)).is(block) && pLevel.getBlockState(pPos.offset(pXOffset, 0, pYOffset + 1)).is(block) && pLevel.getBlockState(pPos.offset(pXOffset + 1, 0, pYOffset + 1)).is(block);
   }
