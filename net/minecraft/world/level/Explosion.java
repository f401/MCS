...
public class Explosion {
   ...
   public Explosion(Level pLevel, @Nullable Entity pSource, double pX, double pY, double pZ, float pRadius, List<BlockPos> pPositions, Explosion.BlockInteraction pBlockInteraction, ParticleOptions pSmallExplosionParticles, ParticleOptions pLargeExplosionParticles, SoundEvent pExplosionSound) {
      this(pLevel, pSource, getDefaultDamageSource(pLevel, pSource), (ExplosionDamageCalculator)null, pX, pY, pZ, pRadius, false, pBlockInteraction, pSmallExplosionParticles, pLargeExplosionParticles, pExplosionSound);
   ...
   public Explosion(Level pLevel, @Nullable Entity pSource, double pX, double pY, double pZ, float pRadius, boolean pFire, Explosion.BlockInteraction pBlockInteraction) {
      this(pLevel, pSource, getDefaultDamageSource(pLevel, pSource), (ExplosionDamageCalculator)null, pX, pY, pZ, pRadius, pFire, pBlockInteraction, ParticleTypes.EXPLOSION, ParticleTypes.EXPLOSION_EMITTER, SoundEvents.GENERIC_EXPLODE);
   }
   ...
   private static LivingEntity getIndirectSourceEntityInternal(@Nullable Entity pSource) {
      if (pSource == null) {
         return null;
      } else if (pSource instanceof PrimedTnt) {
         PrimedTnt primedtnt = (PrimedTnt)pSource;
         return primedtnt.getOwner();
      } else if (pSource instanceof LivingEntity) {
         LivingEntity livingentity = (LivingEntity)pSource;
         return livingentity;
      } else {
         if (pSource instanceof Projectile) {
            Projectile projectile = (Projectile)pSource;
