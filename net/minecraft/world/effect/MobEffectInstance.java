...
public class MobEffectInstance implements Comparable<MobEffectInstance>, net.minecraftforge.common.extensions.IForgeMobEffectInstance {
   ...
   private void writeDetailsTo(CompoundTag pNbt) {
      ...
      this.factorData.ifPresent((p_216903_) -> {
         MobEffectInstance.FactorData.CODEC.encodeStart(NbtOps.INSTANCE, p_216903_).resultOrPartial(LOGGER::error).ifPresent((p_216906_) -> {
            pNbt.put("factor_calculation_data", p_216906_);
         });
      });
   }
   ...
   private static MobEffectInstance loadSpecifiedEffect(MobEffect pEffect, CompoundTag pNbt) {
      ...
      if (pNbt.contains("factor_calculation_data", 10)) {
         optional = MobEffectInstance.FactorData.CODEC.parse(new Dynamic<>(NbtOps.INSTANCE, pNbt.getCompound("factor_calculation_data"))).resultOrPartial(LOGGER::error);
      } else {
   ...
   public int compareTo(MobEffectInstance pOther) {
      int i = 32147;
      return (this.getDuration() <= 32147 || pOther.getDuration() <= 32147) && (!this.isAmbient() || !pOther.isAmbient()) ? ComparisonChain.start().compareFalseFirst(this.isAmbient(), pOther.isAmbient()).compareFalseFirst(this.isInfiniteDuration(), pOther.isInfiniteDuration()).compare(this.getDuration(), pOther.getDuration()).compare(this.getEffect().getSortOrder(this), pOther.getEffect().getSortOrder(this)).result() : ComparisonChain.start().compare(this.isAmbient(), pOther.isAmbient()).compare(this.getEffect().getSortOrder(this), pOther.getEffect().getSortOrder(this)).result();
   }
