...
public class BlockEntitySignDoubleSidedEditableTextFix extends NamedEntityFix {
   ...
   private static <T> Dynamic<T> fixFrontTextTag(Dynamic<T> pTag) {
      Dynamic<T> dynamic = pTag.createString(EMPTY_COMPONENT);
      List<Dynamic<T>> list = getLines(pTag, "Text").map((p_297945_) -> {
         return p_297945_.orElse(dynamic);
      }).toList();
      Dynamic<T> dynamic1 = pTag.emptyMap().set("messages", pTag.createList(list.stream())).set("color", pTag.get("Color").result().orElse(pTag.createString("black"))).set("has_glowing_text", pTag.get("GlowingText").result().orElse(pTag.createBoolean(false))).set("_filtered_correct", pTag.createBoolean(true));
   ...
   private static <T> Stream<Optional<Dynamic<T>>> getLines(Dynamic<T> pDynamic, String pPrefix) {
      return Stream.of(pDynamic.get(pPrefix + "1").result(), pDynamic.get(pPrefix + "2").result(), pDynamic.get(pPrefix + "3").result(), pDynamic.get(pPrefix + "4").result());
   }

   private static <T> Dynamic<T> createDefaultText(Dynamic<T> pDynamic) {
      return pDynamic.emptyMap().set("messages", createEmptyLines(pDynamic)).set("color", pDynamic.createString("black")).set("has_glowing_text", pDynamic.createBoolean(false));
   }

   private static <T> Dynamic<T> createEmptyLines(Dynamic<T> pDynamic) {
      Dynamic<T> dynamic = pDynamic.createString(EMPTY_COMPONENT);
