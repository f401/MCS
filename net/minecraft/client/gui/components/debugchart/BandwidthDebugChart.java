...
public class BandwidthDebugChart extends AbstractDebugChart {
   ...
   protected void renderAdditionalLinesAndLabels(GuiGraphics pGuiGraphics, int pX, int pWidth, int pHeight) {
      ...
      this.drawStringWithShade(pGuiGraphics, toDisplayStringInternal(1048576.0D), pX + 1, pHeight - getSampleHeightInternal(1048576.0D) + 1);
   }
   ...
   private static String toDisplayStringInternal(double pValue) {
      if (pValue >= 1048576.0D) {
         return String.format(Locale.ROOT, "%.1f MiB/s", pValue / 1048576.0D);
      } else {
         return pValue >= 1024.0D ? String.format(Locale.ROOT, "%.1f KiB/s", pValue / 1024.0D) : String.format(Locale.ROOT, "%d B/s", Mth.floor(pValue));
      }
   }
   ...
   private static int getSampleHeightInternal(double pValue) {
      return (int)Math.round(Math.log(pValue + 1.0D) * 60.0D / Math.log(1048576.0D));
   }

   protected int getSampleColor(long pValue) {
      return this.getSampleColor(toBytesPerSecond((double)pValue), 0.0D, -16711681, 8192.0D, -6250241, 1.048576E7D, -65536);
   }

   private static double toBytesPerSecond(double pBytesPerTick) {
      return pBytesPerTick * 20.0D;
   }
}