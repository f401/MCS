...
public class RecipeToast implements Toast {
   ...
   public Toast.Visibility render(GuiGraphics pGuiGraphics, ToastComponent pToastComponent, long pTimeSinceLastVisible) {
      ...
      if (this.recipes.isEmpty()) {
         ...
         RecipeHolder<?> recipeholder = this.recipes.get((int)((double)pTimeSinceLastVisible / Math.max(1.0D, 5000.0D * pToastComponent.getNotificationDisplayTimeMultiplier() / (double)this.recipes.size()) % (double)this.recipes.size()));
