...
public abstract class AbstractSelectionList<E extends AbstractSelectionList.Entry<E>> extends AbstractContainerWidget {
   ...
   public void renderWidget(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
      ...
      if (this.renderBackground) {
         ...
         int i = 32;
         pGuiGraphics.blit(Screen.BACKGROUND_LOCATION, this.getX(), this.getY(), (float)this.getRight(), (float)(this.getBottom() + (int)this.getScrollAmount()), this.width, this.height, 32, 32);
   ...
   public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
      if (!this.isValidMouseClick(pButton)) {
         ...
         if (!this.isMouseOver(pMouseX, pMouseY)) {
            ...
            } else if (this.clickedHeader((int)(pMouseX - (double)(this.getX() + this.width / 2 - this.getRowWidth() / 2)), (int)(pMouseY - (double)this.getY()) + (int)this.getScrollAmount() - 4)) {
               return true;
            }
   ...
   public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pScrollY) {
      this.setScrollAmount(this.getScrollAmount() - pScrollY * (double)this.itemHeight / 2.0D);
      return true;
   }
   ...
   public boolean isMouseOver(double pMouseX, double pMouseY) {
      return pMouseY >= (double)this.getY() && pMouseY <= (double)this.getBottom() && pMouseX >= (double)this.getX() && pMouseX <= (double)this.getRight();
   }
