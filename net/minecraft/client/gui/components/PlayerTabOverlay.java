...
public class PlayerTabOverlay {
      ...
      for(int i4 = 0; i4 < i3; ++i4) {
         ...
         if (i4 < list.size()) {
            ...
            if (flag) {
               ...
               PlayerFaceRenderer.draw(pGuiGraphics, playerinfo1.getSkin().texture(), k2, l2, 8, flag2, flag1);
               k2 += 9;
            }
   ...
   protected void renderPingIcon(GuiGraphics pGuiGraphics, int p_281809_, int p_282801_, int pY, PlayerInfo pPlayerInfo) {
      ...
      pGuiGraphics.blitSprite(resourcelocation, p_282801_ + p_281809_ - 11, pY, 10, 8);
      ...
      if (i > 0) {
         ...
         if (k <= 3) {
            ...
            if (p_282149_ - this.minecraft.font.width(component) >= p_283173_) {
               ...
               component1 = Component.literal("" + f);
            }

            pGuiGraphics.drawString(this.minecraft.font, component1, (p_282149_ + p_283173_ - this.minecraft.font.width(component1)) / 2, pY, j1);
         } else {
            ResourceLocation resourcelocation = flag ? HEART_CONTAINER_BLINKING_SPRITE : HEART_CONTAINER_SPRITE;

            for(int l = i; l < j; ++l) {
               pGuiGraphics.blitSprite(resourcelocation, p_283173_ + l * k, pY, 9, 9);
            }

            for(int i1 = 0; i1 < i; ++i1) {
               pGuiGraphics.blitSprite(resourcelocation, p_283173_ + i1 * k, pY, 9, 9);
               if (flag) {
                  if (i1 * 2 + 1 < playertaboverlay$healthstate.displayedValue()) {
                     pGuiGraphics.blitSprite(HEART_FULL_BLINKING_SPRITE, p_283173_ + i1 * k, pY, 9, 9);
                  }

                  if (i1 * 2 + 1 == playertaboverlay$healthstate.displayedValue()) {
                     pGuiGraphics.blitSprite(HEART_HALF_BLINKING_SPRITE, p_283173_ + i1 * k, pY, 9, 9);
                  }
               }

               if (i1 * 2 + 1 < pHealth) {
                  pGuiGraphics.blitSprite(i1 >= 10 ? HEART_ABSORBING_FULL_BLINKING_SPRITE : HEART_FULL_SPRITE, p_283173_ + i1 * k, pY, 9, 9);
               }

               if (i1 * 2 + 1 == pHealth) {
                  pGuiGraphics.blitSprite(i1 >= 10 ? HEART_ABSORBING_HALF_BLINKING_SPRITE : HEART_HALF_SPRITE, p_283173_ + i1 * k, pY, 9, 9);
               }
            }
