...
public class GuiGraphics implements net.minecraftforge.client.extensions.IForgeGuiGraphics {
   ...
   public void blitSprite(ResourceLocation pSprite, int pSliceWidth, int pSliceHeight, int p_299965_, int p_300008_, int pX, int pY, int pWidth, int pHeight) {
      this.blitSprite(pSprite, pSliceWidth, pSliceHeight, p_299965_, p_300008_, pX, pY, 0, pWidth, pHeight);
   }

   public void blitSprite(ResourceLocation pSprite, int pSliceWidth, int pSliceHeight, int p_299400_, int p_299966_, int pX, int pY, int pBlitOffset, int pWidth, int pHeight) {
      TextureAtlasSprite textureatlassprite = this.sprites.getSprite(pSprite);
      GuiSpriteScaling guispritescaling = this.sprites.getSpriteScaling(textureatlassprite);
      if (guispritescaling instanceof GuiSpriteScaling.Stretch) {
         this.blitSprite(textureatlassprite, pSliceWidth, pSliceHeight, p_299400_, p_299966_, pX, pY, pBlitOffset, pWidth, pHeight);
      } else {
         this.blitSprite(textureatlassprite, pX, pY, pBlitOffset, pWidth, pHeight);
      }
   ...
   private void blitSprite(TextureAtlasSprite pSprite, int pSliceWidth, int pSliceHeight, int p_300994_, int p_297577_, int pX, int pY, int pBlitOffset, int pWidth, int pHeight) {
      if (pWidth != 0 && pHeight != 0) {
         this.innerBlit(pSprite.atlasLocation(), pX, pX + pWidth, pY, pY + pHeight, pBlitOffset, pSprite.getU((float)p_300994_ / (float)pSliceWidth), pSprite.getU((float)(p_300994_ + pWidth) / (float)pSliceWidth), pSprite.getV((float)p_297577_ / (float)pSliceHeight), pSprite.getV((float)(p_297577_ + pHeight) / (float)pSliceHeight));
      }
   }
   ...
   private void blitTiledSprite(TextureAtlasSprite pSprite, int pX, int pY, int pBlitOffset, int pWidth, int pHeight, int p_298797_, int p_299557_, int pSpriteWidth, int pSpriteHeight, int pNineSliceWidth, int pNineSliceHeight) {
      if (pWidth > 0 && pHeight > 0) {
         if (pSpriteWidth > 0 && pSpriteHeight > 0) {
            for(int i = 0; i < pWidth; i += pSpriteWidth) {
               ...
               for(int k = 0; k < pHeight; k += pSpriteHeight) {
                  ...
                  this.blitSprite(pSprite, pNineSliceWidth, pNineSliceHeight, p_298797_, p_299557_, pX + i, pY + k, pBlitOffset, j, l);
               }
            }
