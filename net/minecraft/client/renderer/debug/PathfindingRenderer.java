      ...
      if (p_270481_ && path$debugdata != null) {
         for(Node node1 : path$debugdata.closedSet()) {
            if (distanceToCamera(node1.asBlockPos(), pX, pY, pZ) <= 80.0F) {
               DebugRenderer.renderFilledBox(pPoseStack, pBuffer, (new AABB((double)((float)node1.x + 0.5F - p_270841_ / 2.0F), (double)((float)node1.y + 0.01F), (double)((float)node1.z + 0.5F - p_270841_ / 2.0F), (double)((float)node1.x + 0.5F + p_270841_ / 2.0F), (double)node1.y + 0.1D, (double)((float)node1.z + 0.5F + p_270841_ / 2.0F))).move(-pX, -pY, -pZ), 1.0F, 0.8F, 0.8F, 0.5F);
            }
         }
      ...
      if (p_270748_) {
         for(int j = 0; j < pPath.getNodeCount(); ++j) {
            ...
            if (distanceToCamera(node2.asBlockPos(), pX, pY, pZ) <= 80.0F) {
               DebugRenderer.renderFloatingText(pPoseStack, pBuffer, String.valueOf((Object)node2.type), (double)node2.x + 0.5D, (double)node2.y + 0.75D, (double)node2.z + 0.5D, -1, 0.02F, true, 0.0F, true);
               DebugRenderer.renderFloatingText(pPoseStack, pBuffer, String.format(Locale.ROOT, "%.2f", node2.costMalus), (double)node2.x + 0.5D, (double)node2.y + 0.25D, (double)node2.z + 0.5D, -1, 0.02F, true, 0.0F, true);
            }
         }
      }
