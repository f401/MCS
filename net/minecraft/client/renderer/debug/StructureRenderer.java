...
public class StructureRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   public void addBoundingBox(BoundingBox pBoundingBox, List<StructuresDebugPayload.PieceInfo> pPieces, ResourceKey<Level> pDimension) {
      this.postMainBoxes.computeIfAbsent(pDimension, (p_299944_) -> {
         return new HashMap();
      }).put(pBoundingBox.toString(), pBoundingBox);
      Map<String, StructuresDebugPayload.PieceInfo> map = this.postPieces.computeIfAbsent(pDimension, (p_298617_) -> {
         return new HashMap();
      });
