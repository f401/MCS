...
public class BeeDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   public void removeBeeInfo(int pId) {
      this.beeInfosPerEntity.values().removeIf((p_296269_) -> {
         return p_296269_.id() == pId;
      });
   }
   ...
   private void clearRemovedBees() {
      this.beeInfosPerEntity.entrySet().removeIf((p_296260_) -> {
         return this.minecraft.level.getEntity(p_296260_.getValue().id()) == null;
      });
   }

   private void clearRemovedHives() {
      long i = this.minecraft.level.getGameTime() - 20L;
      this.hives.entrySet().removeIf((p_296254_) -> {
         return p_296254_.getValue().lastSeen() < i;
      });
   }
   ...
   private void doRender(PoseStack pPoseStack, MultiBufferSource pBuffer) {
      ...
      this.hives.values().forEach((p_296259_) -> {
         if (blockpos.closerThan(p_296259_.info.pos(), 30.0D)) {
   ...
   private Map<BlockPos, Set<UUID>> createHiveBlacklistMap() {
      ...
      this.beeInfosPerEntity.values().forEach((p_296271_) -> {
         p_296271_.blacklistedHives().forEach((p_296274_) -> {
            map.computeIfAbsent(p_296274_, (p_173777_) -> {
   ...
   private void renderFlowerInfos(PoseStack pPoseStack, MultiBufferSource pBuffer) {
      ...
      this.beeInfosPerEntity.values().forEach((p_296251_) -> {
         if (p_296251_.flowerPos() != null) {
            map.computeIfAbsent(p_296251_.flowerPos(), (p_296252_) -> {
               return new HashSet();
            }).add(p_296251_.uuid());
         }

      });
      map.forEach((p_296266_, p_296267_) -> {
         Set<String> set = p_296267_.stream().map(DebugEntityNameGenerator::getEntityName).collect(Collectors.toSet());
         int i = 1;
         renderTextOverPos(pPoseStack, pBuffer, set.toString(), p_296266_, i++, -256);
         renderTextOverPos(pPoseStack, pBuffer, "Flower", p_296266_, i++, -1);
         float f = 0.05F;
         DebugRenderer.renderFilledBox(pPoseStack, pBuffer, p_296266_, 0.05F, 0.8F, 0.8F, 0.0F, 0.3F);
      });
   }
   ...
   private Collection<UUID> getHiveMembers(BlockPos pPos) {
      return this.beeInfosPerEntity.values().stream().filter((p_296249_) -> {
         return p_296249_.hasHive(pPos);
   ...
   private Map<BlockPos, List<String>> getGhostHives() {
      ...
      for(BeeDebugPayload.BeeInfo beedebugpayload$beeinfo : this.beeInfosPerEntity.values()) {
         if (beedebugpayload$beeinfo.hivePos() != null && !this.hives.containsKey(beedebugpayload$beeinfo.hivePos())) {
            map.computeIfAbsent(beedebugpayload$beeinfo.hivePos(), (p_113140_) -> {
               return Lists.newArrayList();
            }).add(beedebugpayload$beeinfo.generateName());
         }
      }
