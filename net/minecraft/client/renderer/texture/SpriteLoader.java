...
public class SpriteLoader {
   ...
   public static CompletableFuture<List<SpriteContents>> runSpriteSuppliers(SpriteResourceLoader pSpriteResourceLoader, List<Function<SpriteResourceLoader, SpriteContents>> p_261516_, Executor pExecutor) {
      List<CompletableFuture<SpriteContents>> list = p_261516_.stream().map((p_296294_) -> {
         return CompletableFuture.supplyAsync(() -> {
            return (SpriteContents)p_296294_.apply(pSpriteResourceLoader);
         }, pExecutor);
      }).toList();
   ...
   public CompletableFuture<SpriteLoader.Preparations> loadAndStitch(ResourceManager pResourceManager, ResourceLocation pLocation, int pMipLevel, Executor pExecutor, Collection<MetadataSectionSerializer<?>> pSectionSerializers) {
      ...
      return CompletableFuture.supplyAsync(() -> {
         return SpriteSourceList.load(pResourceManager, pLocation).list(pResourceManager);
      }, pExecutor).thenCompose((p_296297_) -> {
         return runSpriteSuppliers(spriteresourceloader, p_296297_, pExecutor);
      }).thenApply((p_261393_) -> {
