...
public class SectionOcclusionGraph {
   ...
   private void scheduleFullUpdate(boolean pSmartCull, Camera pCamera, Vec3 pCameraPosition) {
      ...
      this.fullUpdateTask = Util.backgroundExecutor().submit(() -> {
         ...
         this.runUpdates(sectionocclusiongraph$graphstate.storage, pCameraPosition, queue, pSmartCull, (p_299279_) -> {
         });
   ...
   private void initializeQueueForFullUpdate(Camera pCamera, Queue<SectionOcclusionGraph.Node> pNodeQueue) {
      ...
      if (sectionrenderdispatcher$rendersection == null) {
         ...
         pNodeQueue.add(new SectionOcclusionGraph.Node(sectionrenderdispatcher$rendersection, (Direction)null, 0));
      }
   ...
   private void runUpdates(SectionOcclusionGraph.GraphStorage pGraphStorage, Vec3 pCameraPosition, Queue<SectionOcclusionGraph.Node> pNodeQueue, boolean pSmartCull, Consumer<SectionRenderDispatcher.RenderSection> pSections) {
      int i = 16;
      BlockPos blockpos = new BlockPos(Mth.floor(pCameraPosition.x / 16.0D) * 16, Mth.floor(pCameraPosition.y / 16.0D) * 16, Mth.floor(pCameraPosition.z / 16.0D) * 16);
         ...
         for(Direction direction : DIRECTIONS) {
            ...
            if (sectionrenderdispatcher$rendersection1 != null && (!pSmartCull || !sectionocclusiongraph$node.hasDirection(direction.getOpposite()))) {
               ...
               if (pSmartCull && flag) {
                  ...
                  while(pCameraPosition.subtract(vec31).lengthSqr() > 3600.0D) {
               ...
               if (sectionocclusiongraph$node1 != null) {
                  ...
                  if (sectionrenderdispatcher$rendersection1.hasAllNeighbors()) {
                     ...
                     pGraphStorage.sectionToNodeMap.put(sectionrenderdispatcher$rendersection1, sectionocclusiongraph$node2);
                     pGraphStorage.chunksWaitingForNeighbors.computeIfAbsent(ChunkPos.asLong(sectionrenderdispatcher$rendersection1.getOrigin()), (p_298371_) -> {
   ...
   protected static class Node {
      ...
      public boolean equals(Object pOther) {
         if (!(pOther instanceof SectionOcclusionGraph.Node sectionocclusiongraph$node)) {
            return false;
         } else {
