...
public abstract class DisplayRenderer<T extends Display, S> extends EntityRenderer<T> {
   ...
   private Quaternionf calculateOrientation(Display.RenderState pRenderState, T pEntity, float pPartialTick, Quaternionf pQuaternion) {
      ...
      switch (pRenderState.billboardConstraints()) {
         case FIXED:
            quaternionf = pQuaternion.rotationYXZ(-0.017453292F * entityYRot(pEntity, pPartialTick), ((float)Math.PI / 180F) * entityXRot(pEntity, pPartialTick), 0.0F);
            break;
         case HORIZONTAL:
            quaternionf = pQuaternion.rotationYXZ(-0.017453292F * entityYRot(pEntity, pPartialTick), ((float)Math.PI / 180F) * cameraXRot(camera), 0.0F);
            break;
         case VERTICAL:
            quaternionf = pQuaternion.rotationYXZ(-0.017453292F * cameraYrot(camera), ((float)Math.PI / 180F) * entityXRot(pEntity, pPartialTick), 0.0F);
            break;
         case CENTER:
            quaternionf = pQuaternion.rotationYXZ(-0.017453292F * cameraYrot(camera), ((float)Math.PI / 180F) * cameraXRot(camera), 0.0F);
            break;
         default:
