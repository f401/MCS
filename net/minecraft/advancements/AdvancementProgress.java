...
public class AdvancementProgress implements Comparable<AdvancementProgress> {
   ...
   private static final Codec<Instant> OBTAINED_TIME_CODEC = ExtraCodecs.temporalCodec(OBTAINED_TIME_FORMAT).xmap(Instant::from, (p_296108_) -> {
      return p_296108_.atZone(ZoneId.systemDefault());
   });
   private static final Codec<Map<String, CriterionProgress>> CRITERIA_CODEC = Codec.unboundedMap(Codec.STRING, OBTAINED_TIME_CODEC).xmap((p_296104_) -> {
      return p_296104_.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, (p_296110_) -> {
         return new CriterionProgress(p_296110_.getValue());
      }));
   }, (p_296101_) -> {
      return p_296101_.entrySet().stream().filter((p_296109_) -> {
         return p_296109_.getValue().isDone();
      }).collect(Collectors.toMap(Map.Entry::getKey, (p_296103_) -> {
         return Objects.requireNonNull(p_296103_.getValue().getObtained());
      }));
   });
   public static final Codec<AdvancementProgress> CODEC = RecordCodecBuilder.create((p_296105_) -> {
      return p_296105_.group(ExtraCodecs.strictOptionalField(CRITERIA_CODEC, "criteria", Map.of()).forGetter((p_296102_) -> {
         return p_296102_.criteria;
      }), Codec.BOOL.fieldOf("done").orElse(true).forGetter(AdvancementProgress::isDone)).apply(p_296105_, (p_296106_, p_296107_) -> {
         return new AdvancementProgress(new HashMap<>(p_296106_));
      });
   });
