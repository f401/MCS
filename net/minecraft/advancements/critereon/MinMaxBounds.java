...
public interface MinMaxBounds<T extends Number> {
   ...
   static <T extends Number, R extends MinMaxBounds<T>> Codec<R> createCodec(Codec<T> pCodec, MinMaxBounds.BoundsFactory<T, R> pBoundsFactory) {
      Codec<R> codec = RecordCodecBuilder.create((p_299567_) -> {
         return p_299567_.group(ExtraCodecs.strictOptionalField(pCodec, "min").forGetter(MinMaxBounds::min), ExtraCodecs.strictOptionalField(pCodec, "max").forGetter(MinMaxBounds::max)).apply(p_299567_, pBoundsFactory::create);
      });
      return Codec.either(codec, pCodec).xmap((p_299505_) -> {
         return p_299505_.map((p_300863_) -> {
            return p_300863_;
         }, (p_300712_) -> {
            return pBoundsFactory.create(Optional.of(p_300712_), Optional.of(p_300712_));
         });
      }, (p_300263_) -> {
   ...
   static <T extends Number, R extends MinMaxBounds<T>> R fromReader(StringReader pReader, MinMaxBounds.BoundsFromReaderFactory<T, R> pBoundedFactory, Function<String, T> pValueFactory, Supplier<DynamicCommandExceptionType> pCommandExceptionSupplier, Function<T, T> pFormatter) throws CommandSyntaxException {
      if (!pReader.canRead()) {
   ...
   public static record Doubles(Optional<Double> min, Optional<Double> max, Optional<Double> minSq, Optional<Double> maxSq) implements MinMaxBounds<Double> {
      ...
      public static MinMaxBounds.Doubles fromJson(@Nullable JsonElement pJson) {
         return pJson != null && !pJson.isJsonNull() ? Util.getOrThrow(CODEC.parse(JsonOps.INSTANCE, pJson), JsonParseException::new) : ANY;
      }
      ...
      public static MinMaxBounds.Doubles fromReader(StringReader pReader, Function<Double, Double> pFormatter) throws CommandSyntaxException {
         return MinMaxBounds.fromReader(pReader, MinMaxBounds.Doubles::create, Double::parseDouble, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidDouble, pFormatter);
      }
   ...
   public static record Ints(Optional<Integer> min, Optional<Integer> max, Optional<Long> minSq, Optional<Long> maxSq) implements MinMaxBounds<Integer> {
      ...
      public static MinMaxBounds.Ints fromReader(StringReader pReader, Function<Integer, Integer> pValueFunction) throws CommandSyntaxException {
         return MinMaxBounds.fromReader(pReader, MinMaxBounds.Ints::create, Integer::parseInt, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidInt, pValueFunction);
      }
