...
public record TagPredicate<T>(TagKey<T> tag, boolean expected) {
   public static <T> Codec<TagPredicate<T>> codec(ResourceKey<? extends Registry<T>> pRegistryKey) {
      return RecordCodecBuilder.create((p_299938_) -> {
         return p_299938_.group(TagKey.codec(pRegistryKey).fieldOf("id").forGetter(TagPredicate::tag), Codec.BOOL.fieldOf("expected").forGetter(TagPredicate::expected)).apply(p_299938_, TagPredicate::new);
      });
   }
