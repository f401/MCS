...
public record ItemPredicate(Optional<TagKey<Item>> tag, Optional<HolderSet<Item>> items, MinMaxBounds.Ints count, MinMaxBounds.Ints durability, List<EnchantmentPredicate> enchantments, List<EnchantmentPredicate> storedEnchantments, Optional<Holder<Potion>> potion, Optional<NbtPredicate> nbt) {
   ...
   public static Optional<ItemPredicate> fromJson(@Nullable JsonElement pJson) {
   ...
   public static JsonElement serializeToJsonArray(List<ItemPredicate> pPredicates) {
      return Util.getOrThrow(CODEC.listOf().encodeStart(JsonOps.INSTANCE, pPredicates), IllegalStateException::new);
   }

   public static List<ItemPredicate> fromJsonArray(@Nullable JsonElement pJson) {
      return pJson != null && !pJson.isJsonNull() ? Util.getOrThrow(CODEC.listOf().parse(JsonOps.INSTANCE, pJson), JsonParseException::new) : List.of();
   }
