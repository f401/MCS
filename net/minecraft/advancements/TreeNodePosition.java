...
public class TreeNodePosition {
   ...
   private void finalizePosition() {
      this.node.advancement().display().ifPresent((p_298323_) -> {
         p_298323_.setLocation((float)this.x, this.y);
      });
      if (!this.children.isEmpty()) {
         for(TreeNodePosition treenodeposition : this.children) {
   ...
   public static void run(AdvancementNode pRootNode) {
      if (pRootNode.advancement().display().isEmpty()) {
         ...
         TreeNodePosition treenodeposition = new TreeNodePosition(pRootNode, (TreeNodePosition)null, (TreeNodePosition)null, 1, 0);
