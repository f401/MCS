...
public class WorldLoader {
   ...
   public static <D, R> CompletableFuture<R> load(WorldLoader.InitConfig pInitConfig, WorldLoader.WorldDataSupplier<D> pWorldDataSupplier, WorldLoader.ResultFactory<D, R> pResultFactory, Executor pBackgroundExecutor, Executor pGameExecutor) {
      try {
         ...
         return ReloadableServerResources.loadResources(closeableresourcemanager, registryaccess$frozen2, worlddataconfiguration.enabledFeatures(), pInitConfig.commandSelection(), pInitConfig.functionCompilationLevel(), pBackgroundExecutor, pGameExecutor).whenComplete((p_214370_, p_214371_) -> {
            if (p_214371_ != null) {
