   ...
   public T detectPackResources(Path pPath, List<ForbiddenSymlinkInfo> pForbiddenSymlinkInfos) throws IOException {
      ...
      if (basicfileattributes.isDirectory()) {
         ...
         if (!pForbiddenSymlinkInfos.isEmpty()) {
            ...
            return (T)(!Files.isRegularFile(path.resolve("pack.mcmeta")) ? null : this.createDirectoryPack(path));
         }
      } else {
         return (T)(basicfileattributes.isRegularFile() && path.getFileName().toString().endsWith(".zip") ? this.createZipPack(path) : null);
      }
   }
