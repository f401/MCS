...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
   ...
   private CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> scheduleChunkLoad(ChunkPos pChunkPos) {
      return this.readChunk(pChunkPos).thenApply((p_214925_) -> {
         ...
         if (p_308939_.isPresent()) {
            ChunkAccess chunkaccess = ChunkSerializer.read(this.level, this.poiManager, pChunkPos, p_308939_.get());
   ...
   private void applyChunkTrackingView(ServerPlayer pPlayer, ChunkTrackingView pChunkTrackingView) {
      if (pPlayer.level() == this.level) {
         ...
         ChunkTrackingView.difference(chunktrackingview, pChunkTrackingView, (p_296566_) -> {
            this.markChunkPendingToSend(pPlayer, p_296566_);
         }, (p_296568_) -> {
            dropChunk(pPlayer, p_296568_);
         });
