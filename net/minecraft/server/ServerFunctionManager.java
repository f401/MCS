...
public class ServerFunctionManager {
   ...
   public void execute(CommandFunction<CommandSourceStack> pFunction, CommandSourceStack pSource) {
      ...
      profilerfiller.push(() -> {
         return "function " + pFunction.id();
      });

      try {
         InstantiatedFunction<CommandSourceStack> instantiatedfunction = pFunction.instantiate((CompoundTag)null, this.getDispatcher(), pSource);
         Commands.executeCommandInContext(pSource, (p_311172_) -> {
            ExecutionContext.queueInitialFunctionCall(p_311172_, instantiatedfunction, pSource, CommandResultCallback.EMPTY);
         });
