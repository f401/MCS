...
public class ServerFunctionManager {
   ...
   public int execute(CommandFunction pFunctionObject, CommandSourceStack pSource) {
      try {
         return this.execute(pFunctionObject, pSource, (ServerFunctionManager.TraceCallbacks)null, (CompoundTag)null);
      } catch (FunctionInstantiationException functioninstantiationexception) {
         return 0;
      }
   }

   public int execute(CommandFunction pFunctionObject, CommandSourceStack pSource, @Nullable ServerFunctionManager.TraceCallbacks pTracer, @Nullable CompoundTag pArguments) throws FunctionInstantiationException {
      CommandFunction commandfunction = pFunctionObject.instantiate(pArguments, this.getDispatcher(), pSource);
      if (this.context != null) {
         if (pTracer != null) {
            this.context.reportError(NO_RECURSIVE_TRACES.getString());
            return 0;
         } else {
            this.context.delayFunctionCall(commandfunction, pSource);
            return 0;
         }
      } else {
         int i;
         try {
            ...
            i = this.context.runTopCommand(commandfunction, pSource);
         } finally {
            this.context = null;
         }
