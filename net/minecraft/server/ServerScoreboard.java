...
public class ServerScoreboard extends Scoreboard {
   ...
   protected void onScoreChanged(ScoreHolder pScoreHolder, Objective pObjective, Score pScore) {
      ...
      if (this.trackedObjectives.contains(pObjective)) {
         this.server.getPlayerList().broadcastAll(new ClientboundSetScorePacket(pScoreHolder.getScoreboardName(), pObjective.getName(), pScore.value(), pScore.display(), pScore.numberFormat()));
      }
   ...
   public void onPlayerRemoved(ScoreHolder pScoreHolder) {
      ...
      this.server.getPlayerList().broadcastAll(new ClientboundResetScorePacket(pScoreHolder.getScoreboardName(), (String)null));
      this.setDirty();
   }
   ...
   public List<Packet<?>> getStartTrackingPackets(Objective pObjective) {
      ...
      for(PlayerScoreEntry playerscoreentry : this.listPlayerScores(pObjective)) {
         list.add(new ClientboundSetScorePacket(playerscoreentry.owner(), pObjective.getName(), playerscoreentry.value(), playerscoreentry.display(), playerscoreentry.numberFormatOverride()));
      }
