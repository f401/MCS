...
public class StopSoundCommand {
   ...
   private static int stopSound(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, @Nullable SoundSource pCategory, @Nullable ResourceLocation pSound) {
      ...
      if (pCategory != null) {
         if (pSound != null) {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.stopsound.success.source.sound", Component.translationArg(pSound), pCategory.getName());
            }, true);
         } else {
            pSource.sendSuccess(() -> {
         ...
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.stopsound.success.sourceless.sound", Component.translationArg(pSound));
         }, true);
      } else {
         pSource.sendSuccess(() -> {
