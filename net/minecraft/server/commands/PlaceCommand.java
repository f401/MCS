...
public class PlaceCommand {
   ...
   public static int placeTemplate(CommandSourceStack pSource, ResourceLocation pTemplate, BlockPos pPos, Rotation pRotation, Mirror pMirror, float pIntegrity, int pSeed) throws CommandSyntaxException {
      ...
      if (optional.isEmpty()) {
         ...
         if (!flag) {
            ...
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.place.template.success", Component.translationArg(pTemplate), pPos.getX(), pPos.getY(), pPos.getZ());
            }, true);
            return 1;
         }
      }
   ...
   private static void checkLoaded(ServerLevel pLevel, ChunkPos pStart, ChunkPos pEnd) throws CommandSyntaxException {
      if (ChunkPos.rangeClosed(pStart, pEnd).filter((p_308780_) -> {
         return !pLevel.isLoaded(p_308780_.getWorldPosition());
      }).findAny().isPresent()) {
