...
public class ReturnCommand {
   public static <T extends ExecutionCommandSource<T>> void register(CommandDispatcher<T> pDispatcher) {
      pDispatcher.register(LiteralArgumentBuilder.<T>literal("return").requires((p_308799_) -> {
         return p_308799_.hasPermission(2);
      }).then(RequiredArgumentBuilder.<T, Integer>argument("value", IntegerArgumentType.integer()).executes(new ReturnCommand.ReturnValueCustomExecutor<>())).then(LiteralArgumentBuilder.<T>literal("fail").executes(new ReturnCommand.ReturnFailCustomExecutor<>())).then(LiteralArgumentBuilder.<T>literal("run").forward(pDispatcher.getRoot(), new ReturnCommand.ReturnFromCommandCustomModifier<>(), false)));
   }
