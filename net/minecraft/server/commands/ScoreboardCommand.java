...
public class ScoreboardCommand {
   ...
   private static ArgumentBuilder<CommandSourceStack, ?> addNumberFormats(ArgumentBuilder<CommandSourceStack, ?> pBuilder, ScoreboardCommand.NumberFormatCommandExecutor pCommandExecutor) {
      return pBuilder.then(Commands.literal("blank").executes((p_308836_) -> {
         return pCommandExecutor.run(p_308836_, BlankFormat.INSTANCE);
         ...
         return pCommandExecutor.run(p_308824_, new FixedFormat(component));
         ...
         return pCommandExecutor.run(p_308869_, new StyledFormat(style));
      }))).executes((p_308875_) -> {
         return pCommandExecutor.run(p_308875_, (NumberFormat)null);
      });
   }
   ...
   private static int getScore(CommandSourceStack pSource, ScoreHolder pScoreHolder, Objective pObjective) throws CommandSyntaxException {
      ...
      if (readonlyscoreinfo == null) {
         ...
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.get.success", pScoreHolder.getFeedbackDisplayName(), readonlyscoreinfo.value(), pObjective.getFormattedDisplayName());
         }, false);
   ...
   private static int performOperation(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pTargetObjectives, OperationArgument.Operation pOperation, Collection<ScoreHolder> pSourceEntities, Objective pSourceObjective) throws CommandSyntaxException {
      Scoreboard scoreboard = pSource.getServer().getScoreboard();
      int i = 0;
      ...
      if (pTargets.size() == 1) {
         int j = i;
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.operation.success.single", pTargetObjectives.getFormattedDisplayName(), getFirstTargetName(pTargets), j);
         }, true);
      } else {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.operation.success.multiple", pTargetObjectives.getFormattedDisplayName(), pTargets.size());
         }, true);
      }
   ...
   private static int enableTrigger(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pObjective) throws CommandSyntaxException {
      if (pObjective.getCriteria() != ObjectiveCriteria.TRIGGER) {
         ...
         if (i == 0) {
            ...
            if (pTargets.size() == 1) {
               pSource.sendSuccess(() -> {
                  return Component.translatable("commands.scoreboard.players.enable.success.single", pObjective.getFormattedDisplayName(), getFirstTargetName(pTargets));
               }, true);
            } else {
               pSource.sendSuccess(() -> {
   ...
   private static int resetScores(CommandSourceStack pSource, Collection<ScoreHolder> pTargets) {
      ...
      if (pTargets.size() == 1) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.reset.all.single", getFirstTargetName(pTargets));
         }, true);
      } else {
         pSource.sendSuccess(() -> {
   ...
   private static int resetScore(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pObjective) {
      ...
      if (pTargets.size() == 1) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.reset.specific.single", pObjective.getFormattedDisplayName(), getFirstTargetName(pTargets));
         }, true);
      } else {
         pSource.sendSuccess(() -> {
   ...
   private static int setScore(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pObjective, int pNewValue) {
      ...
      if (pTargets.size() == 1) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.set.success.single", pObjective.getFormattedDisplayName(), getFirstTargetName(pTargets), pNewValue);
         }, true);
      } else {
         pSource.sendSuccess(() -> {
   ...
   private static int setScoreDisplay(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pObjective, @Nullable Component pDisplayName) {
      ...
      if (pDisplayName == null) {
         if (pTargets.size() == 1) {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.scoreboard.players.display.name.clear.success.single", getFirstTargetName(pTargets), pObjective.getFormattedDisplayName());
            }, true);
         } else {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.scoreboard.players.display.name.clear.success.multiple", pTargets.size(), pObjective.getFormattedDisplayName());
            }, true);
         }
      } else if (pTargets.size() == 1) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.display.name.set.success.single", pDisplayName, getFirstTargetName(pTargets), pObjective.getFormattedDisplayName());
         }, true);
      } else {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.display.name.set.success.multiple", pDisplayName, pTargets.size(), pObjective.getFormattedDisplayName());
         }, true);
      }
   ...
   private static int setScoreNumberFormat(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pObjective, @Nullable NumberFormat pNumberFormat) {
      ...
      if (pNumberFormat == null) {
         if (pTargets.size() == 1) {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.scoreboard.players.display.numberFormat.clear.success.single", getFirstTargetName(pTargets), pObjective.getFormattedDisplayName());
            }, true);
         } else {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.scoreboard.players.display.numberFormat.clear.success.multiple", pTargets.size(), pObjective.getFormattedDisplayName());
            }, true);
         }
      } else if (pTargets.size() == 1) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.display.numberFormat.set.success.single", getFirstTargetName(pTargets), pObjective.getFormattedDisplayName());
         }, true);
      } else {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.display.numberFormat.set.success.multiple", pTargets.size(), pObjective.getFormattedDisplayName());
         }, true);
      }
   ...
   private static int addScore(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pObjective, int pAmount) {
      ...
      if (pTargets.size() == 1) {
         int j = i;
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.add.success.single", pAmount, pObjective.getFormattedDisplayName(), getFirstTargetName(pTargets), j);
         }, true);
      } else {
         pSource.sendSuccess(() -> {
   ...
   private static int removeScore(CommandSourceStack pSource, Collection<ScoreHolder> pTargets, Objective pObjective, int pAmount) {
      ...
      if (pTargets.size() == 1) {
         int j = i;
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.remove.success.single", pAmount, pObjective.getFormattedDisplayName(), getFirstTargetName(pTargets), j);
         }, true);
      } else {
         pSource.sendSuccess(() -> {
   ...
   private static int listTrackedPlayerScores(CommandSourceStack pSource, ScoreHolder pScore) {
      ...
      if (object2intmap.isEmpty()) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.list.entity.empty", pScore.getFeedbackDisplayName());
         }, false);
      } else {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.players.list.entity.success", pScore.getFeedbackDisplayName(), object2intmap.size());
         }, false);
   ...
   private static int clearDisplaySlot(CommandSourceStack pSource, DisplaySlot pSlot) throws CommandSyntaxException {
      ...
      if (scoreboard.getDisplayObjective(pSlot) == null) {
         ...
         scoreboard.setDisplayObjective(pSlot, (Objective)null);
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.objectives.display.cleared", pSlot.getSerializedName());
         }, true);
         return 0;
      }
   }
   ...
   private static int setDisplaySlot(CommandSourceStack pSource, DisplaySlot pSlot, Objective pObjective) throws CommandSyntaxException {
      ...
      if (scoreboard.getDisplayObjective(pSlot) == pObjective) {
         ...
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.objectives.display.set", pSlot.getSerializedName(), pObjective.getDisplayName());
         }, true);
         return 0;
      }
   }
   ...
   private static int setDisplayAutoUpdate(CommandSourceStack pSource, Objective pObjective, boolean pDisplayAutoUpdate) {
      if (pObjective.displayAutoUpdate() != pDisplayAutoUpdate) {
         ...
         if (pDisplayAutoUpdate) {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.scoreboard.objectives.modify.displayAutoUpdate.enable", pObjective.getName(), pObjective.getFormattedDisplayName());
            }, true);
         } else {
            pSource.sendSuccess(() -> {
               return Component.translatable("commands.scoreboard.objectives.modify.displayAutoUpdate.disable", pObjective.getName(), pObjective.getFormattedDisplayName());
            }, true);
         }
      }
   ...
   private static int setObjectiveFormat(CommandSourceStack pSource, Objective pObjective, @Nullable NumberFormat pFormat) {
      ...
      if (pFormat != null) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.objectives.modify.objectiveFormat.set", pObjective.getName());
         }, true);
      } else {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.objectives.modify.objectiveFormat.clear", pObjective.getName());
         }, true);
      }
   ...
   private static int addObjective(CommandSourceStack pSource, String pName, ObjectiveCriteria pCriteria, Component pDisplayName) throws CommandSyntaxException {
      ...
      if (scoreboard.getObjective(pName) != null) {
         ...
         scoreboard.addObjective(pName, pCriteria, pDisplayName, pCriteria.getDefaultRenderType(), false, (NumberFormat)null);
