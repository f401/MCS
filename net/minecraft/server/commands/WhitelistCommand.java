...
public class WhitelistCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> pDispatcher) {
      pDispatcher.register(Commands.literal("whitelist").requires((p_139234_) -> {
         ...
         return SharedSuggestionProvider.suggest(playerlist.getPlayers().stream().filter((p_296550_) -> {
            return !playerlist.getWhiteList().isWhiteListed(p_296550_.getGameProfile());
         }).map((p_296552_) -> {
            return p_296552_.getGameProfile().getName();
         }), p_139217_);
      }).executes((p_139224_) -> {
