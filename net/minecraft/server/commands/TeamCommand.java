...
public class TeamCommand {
   ...
   private static int leaveTeam(CommandSourceStack pSource, Collection<ScoreHolder> pPlayers) {
      ...
      if (pPlayers.size() == 1) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.team.leave.success.single", getFirstMemberName(pPlayers));
         }, true);
      } else {
         pSource.sendSuccess(() -> {
   ...
   private static int joinTeam(CommandSourceStack pSource, PlayerTeam pTeam, Collection<ScoreHolder> pPlayers) {
      ...
      if (pPlayers.size() == 1) {
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.team.join.success.single", getFirstMemberName(pPlayers), pTeam.getFormattedDisplayName());
         }, true);
      } else {
         pSource.sendSuccess(() -> {
