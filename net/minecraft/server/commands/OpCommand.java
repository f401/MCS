...
public class OpCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> pDispatcher) {
      pDispatcher.register(Commands.literal("op").requires((p_138087_) -> {
         ...
         return SharedSuggestionProvider.suggest(playerlist.getPlayers().stream().filter((p_296521_) -> {
            return !playerlist.isOp(p_296521_.getGameProfile());
         }).map((p_296519_) -> {
            return p_296519_.getGameProfile().getName();
         }), p_138085_);
      }).executes((p_138082_) -> {
