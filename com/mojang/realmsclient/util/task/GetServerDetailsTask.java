...
public class GetServerDetailsTask extends LongRunningTask {
   ...
   private static UUID generatePackId(RealmsServer pRealmsServer) {
      return pRealmsServer.minigameName != null ? UUID.nameUUIDFromBytes(("minigame:" + pRealmsServer.minigameName).getBytes(StandardCharsets.UTF_8)) : UUID.nameUUIDFromBytes(("realms:" + pRealmsServer.name + ":" + pRealmsServer.activeSlot).getBytes(StandardCharsets.UTF_8));
   }
   ...
   private RealmsLongConfirmationScreen resourcePackDownloadConfirmationScreen(RealmsServerAddress pServerAddress, UUID pId, Function<RealmsServerAddress, Screen> pConnectScreenGetter) {
      BooleanConsumer booleanconsumer = (p_167645_) -> {
         if (!p_167645_) {
            ...
            this.scheduleResourcePackDownload(pServerAddress, pId).thenRun(() -> {
               setScreen(pConnectScreenGetter.apply(pServerAddress));
